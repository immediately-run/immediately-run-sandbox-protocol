 ...
 * built this in the sandbox repo, R3-274b1 moved it here so both sides CONSUME it
 * instead of one side copying it to the other).
 *
 * `src/descriptors.protocol.mjs` is the single definition of the sandbox↔SDK wire
 * vocabulary. This projects it into:
 *
 *   src/sandbox.ts            the frame's constants + payload types
 *   src/sdk.ts                the SDK's constants + payload types
 *   snapshots/sandbox.json    the frame's snapshot, for that repo's gate
 *   snapshots/sdk.json        the SDK's snapshot, for that repo's gate
 *
...
import {
  ...
  FORMAT_VERSION,
  REPOS,
} from '../src/descriptors.protocol.mjs';
...
const descriptorsHash = createHash('sha256')
  .update(readFileSync(join(repoRoot, 'src/descriptors.protocol.mjs')))
  .digest('hex')
  .slice(0, 16);
...
const emitTs = (side) => {
  ...
  const out = [];
  out.push(`// GENERATED by scripts/generate.mjs — DO NOT EDIT.`);
  out.push(`// Source of truth: src/descriptors.protocol.mjs (@immediately-run/sandbox-protocol)`);
...
const written = [
  write('src/sandbox.ts', emitTs('sandbox')),
  write('src/sdk.ts', emitTs('sdk')),
  write('snapshots/sandbox.json', json(snapshotFor('sandbox'))),
  write('snapshots/sdk.json', json(snapshotFor('sdk'))),
];
