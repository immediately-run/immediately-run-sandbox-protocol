 ...
 *
 * Run: node scripts/verify-drift.mjs [--self-test]   (npm run check:drift)
 */
...
const OUTPUTS = [
  'src/sandbox.ts',
  'src/sdk.ts',
  'snapshots/sandbox.json',
  'snapshots/sdk.json',
];

for (const rel of OUTPUTS) {
  if (!existsSync(join(root, rel))) {
    console.error(`error: ${rel} missing — run \`npm run generate\`.`);
    process.exit(1);
  }
}
...
const regenerate = () => {
  ...
  try {
    cpSync(here, join(tmp, 'scripts'), { recursive: true });
    cpSync(join(root, 'src/descriptors.protocol.mjs'), join(tmp, 'src/descriptors.protocol.mjs'), {
      recursive: false,
    });
    execFileSync(process.execPath, [join(tmp, 'scripts/generate.mjs'), '--out', tmp], {
      cwd: tmp,
      stdio: 'pipe',
    });
...
const selfTest = () => {
  ...
  const cases = [
    [
      ...
      { ...real, 'src/sandbox.ts': real['src/sandbox.ts'].replace("export const THEME = 'theme';", "export const THEME = 'host-theme';") },
    ],
    [
      'a hand-edited wire name in the snapshot',
      { ...real, 'snapshots/sandbox.json': real['snapshots/sandbox.json'].replace('"mount-add"', '"mount-added"') },
    ],
    [
      'a hand-edited SDK-side module',
      { ...real, 'src/sdk.ts': real['src/sdk.ts'] + '\nexport const sneaked = 1;\n' },
    ],
    [
      'a hand-edited SDK-side snapshot',
      { ...real, 'snapshots/sdk.json': real['snapshots/sdk.json'].replace('"optional": false', '"optional": true') },
    ],
    [
      'a deleted line',
      { ...real, 'src/sandbox.ts': real['src/sandbox.ts'].split('\n').filter((_, i) => i !== 20).join('\n') },
    ],
  ];
